-- =====================================================
-- G-ADMIN MINI: RLS ENHANCEMENTS & IMPROVEMENTS
...
BEGIN
    has_access := false;
    user_current_role := public.get_user_role_enhanced();
    ...
    PERFORM public.log_security_event(
        ...
        'read',
        has_access,
        CASE WHEN NOT has_access THEN format('Insufficient role: %s < %s', user_current_role, required_role) ELSE NULL END,
        jsonb_build_object(
...
-- Function to create role review records
CREATE OR REPLACE FUNCTION public.schedule_role_review(
  p_user_id UUID,
  p_review_frequency_days INTEGER DEFAULT 90
)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SELECT role INTO v_user_role FROM public.users_roles WHERE user_id = p_user_id AND is_active = true;
  
  IF v_user_role IS NULL THEN
    RAISE EXCEPTION 'User % has no active role', p_user_id;
  END IF;
  
  -- Insert or update review schedule
  INSERT INTO public.role_review_schedule (
    user_id, role, assigned_date, next_review_date, review_frequency_days
  ) VALUES (
    p_user_id, v_user_role, NOW(), NOW() + INTERVAL '1 day' * p_review_frequency_days, p_review_frequency_days
  )
  ...
  PERFORM public.log_security_event(
    ...
    true,
    NULL,
    jsonb_build_object('role', v_user_role, 'frequency_days', p_review_frequency_days)
  );
END;
$$;
...
BEGIN
  ...
  PERFORM public.schedule_role_review(
    p_target_user_id,
    CASE p_new_role
      WHEN 'SUPER_ADMIN' THEN 30
      WHEN 'ADMINISTRADOR' THEN 60
      WHEN 'SUPERVISOR' THEN 90
      ELSE 180
    END
  );
...
-- =====================================================
-- 5. AUTOMATED TRIGGERS AND MAINTENANCE
-- =====================================================

-- Trigger to automatically schedule role reviews for new roles
CREATE OR REPLACE FUNCTION public.trigger_schedule_role_review()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
...
CREATE TRIGGER trigger_auto_schedule_role_review
  AFTER INSERT OR UPDATE ON public.users_roles
  FOR EACH ROW
  EXECUTE FUNCTION trigger_schedule_role_review();
...
SELECT 'G-Admin Mini RLS Enhancements successfully created!' as result;