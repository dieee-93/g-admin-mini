...
import { calculateLaborCosts, getLaborCostSummary, type LaborCostData, type LaborCostSummary } from '@/modules/team/services';
...
export function LaborCostDashboard({
  ...
  const loadLaborCostData = async () => {
    ...
    try {
      const { getCostPerHourAnalysis } = await import('@/modules/team/services');
      const [costs, summary, analysis] = await Promise.all([
        calculateLaborCosts(currentDateRange.start, currentDateRange.end),
        getLaborCostSummary(currentDateRange.start, currentDateRange.end),
      ...
      {showDetails && (
        ...
        >
          <Tabs.List>
            ...
            <Tabs.Trigger value="teamMembers">Por Empleado</Tabs.Trigger>
                        ...
                        <HStack justify="space-between" align="center">
                          ...
                          <VStack gap="2" align="end">
                            <Badge colorPalette={getEfficiencyColor(dept.efficiency_score * 10)}>
                              {dept.teamMembers} empleado{dept.teamMembers > 1 ? 's' : ''}
                            </Badge>
                            <Text fontSize="sm" color="gray.600">
