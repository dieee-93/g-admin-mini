...
import { cleanupUtils, testConfigs } from '../helpers/test-utilities';
import { mockEventTemplates, mockBusinessData } from '../helpers/mock-data';
import type { NamespacedEvent, DeduplicationMetadata, EventBusConfig } from '../../types';

// Mock DeduplicationStore for testing
class MockDeduplicationStore {
  private data = new Map<string, DeduplicationMetadata>();
  ...
  async store(metadata: DeduplicationMetadata): Promise<void> {
    // Preserve original timestamp, only add if not present
    const metadataWithTimestamp = {
      ...metadata,
      timestamp: metadata.timestamp || Date.now()
    };
    this.data.set(metadata.contentHash, metadataWithTimestamp);
  }
  
  async findByContentHash(contentHash: string): Promise<DeduplicationMetadata | null> {
    return this.data.get(contentHash) || null;
  }
  
  async findByOperationId(operationId: string): Promise<DeduplicationMetadata | null> {
    for (const metadata of this.data.values()) {
  ...
  async findBySemanticKey(semanticKey: string, withinMs: number): Promise<DeduplicationMetadata[]> {
    ...
    const results: DeduplicationMetadata[] = [];
    
    for (const metadata of this.data.values()) {
      if (metadata.semanticKey === semanticKey && 
          (metadata.timestamp || Date.now()) > cutoffTime) {
  ...
  async cleanup(olderThanMs: number): Promise<number> {
    ...
    for (const [key, metadata] of this.data.entries()) {
      if ((metadata.timestamp || 0) < cutoffTime) {
...
describe('DeduplicationManager', () => {
  ...
  beforeEach(async () => {
    ...
    const mockStore = new MockDeduplicationStore() as any;
    deduplicationManager = new DeduplicationManager({
      deduplicationEnabled: true,
      testModeEnabled: true,
      storeName: 'test-dedup-store',
      defaultDeduplicationWindow: 300000, // 5 minutes
  ...
  describe('Configuration', () => {
    it('should respect deduplication window configuration', async () => {
      const customManager = new DeduplicationManager({
        ...testConfigs.unit,
        deduplicationEnabled: true,
        defaultDeduplicationWindow: 1000, // 1 second
        cleanupIntervalMs: 60000
      } as EventBusConfig);
    ...
    it('should disable deduplication when configured', async () => {
      const disabledManager = new DeduplicationManager({
        ...
        defaultDeduplicationWindow: 300000,
        cleanupIntervalMs: 60000
      } as EventBusConfig);
