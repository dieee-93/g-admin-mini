// EventBus.test.ts - Unit tests for EventBus core functionality
...
describe('EventBus Core Functionality', () => {
  ...
  describe('Metrics and Monitoring', () => {
    it('should collect basic metrics', async () => {
      ...
      expect(metrics.avgLatencyMs).toBeGreaterThanOrEqual(0); // Changed to >= 0 since it might be 0 for very fast events
    ...
    it('should track event patterns', async () => {
      ...
      expect(metrics.eventPatterns[pattern1]?.totalEvents).toBe(2);
      expect(metrics.eventPatterns[pattern2]?.totalEvents).toBe(1);
    });
  });
