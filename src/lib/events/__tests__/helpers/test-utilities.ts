// Test utilities - Helper functions for EventBus testing
...
export const testConfigs = {
  ...
  unit: {
    ...
    security: {
      enableEncryption: false // Disable encryption in tests
    }
  } as Partial<EventBusConfig>,

  // Config for integration tests
  integration: {
    persistenceEnabled: false, // Disabled for tests - IndexedDB issues in test env
    deduplicationEnabled: false, // Disabled - causes hanging with persistenceEnabled: false
    offlineSyncEnabled: false, // Disabled to avoid external dependencies
    testModeEnabled: true,
    metricsEnabled: false, // Disabled - metrics collection blocks test execution
  ...
  // Config for health monitoring tests  
  health: {
    persistenceEnabled: false, // Disabled for tests - IndexedDB issues in test env
    deduplicationEnabled: false, // Disabled - causes hanging with persistenceEnabled: false
    offlineSyncEnabled: false, // Disabled to avoid external dependencies
    testModeEnabled: true,
    metricsEnabled: true, // Enabled for health monitoring tests
    healthCheckIntervalMs: 1000, // Faster interval for tests
    maxStorageSize: 10 * 1024 * 1024, // 10MB for tests
    maxEventHistorySize: 1000
  } as Partial<EventBusConfig>,
  ...
  performance: {
    persistenceEnabled: false, // Disabled for tests - IndexedDB hangs in test environment
    deduplicationEnabled: false, // Disabled - requires persistence
    offlineSyncEnabled: false,
    testModeEnabled: true,
    metricsEnabled: true,
    maxConcurrentEvents: 1000,
    healthCheckIntervalMs: 5000,
    maxStorageSize: 50 * 1024 * 1024, // 50MB for performance tests
  ...
  stress: {
    persistenceEnabled: false, // Disabled for tests - IndexedDB hangs in test environment
    deduplicationEnabled: false, // Disabled - allows duplicates in stress tests + requires persistence
    offlineSyncEnabled: false,
    testModeEnabled: true,
    metricsEnabled: true,
    ...
    maxStorageSize: 100 * 1024 * 1024, // 100MB for stress tests
...
export class TestSetup {
  private static instances = new Set<EventBus>();
  private static harnesses = new Set<EventBusTestingHarness>();

  // Create EventBus instance for testing
  static async createEventBus(config?: Partial<EventBusConfig>): Promise<EventBus> {
  ...
  static createHarness(eventBus?: EventBus, config?: Partial<EventBusConfig>): EventBusTestingHarness {
