...
describe('EventStore', () => {
  let eventStore: EventStoreIndexedDB;
  const testStoreName = 'test-eventbus-store';

  beforeEach(async () => {
    // Clear any existing test data
    await cleanupUtils.clearStorage();
    
    eventStore = new EventStoreIndexedDB({
  ...
  describe('Event Retrieval', () => {
    beforeEach(async () => {
      ...
      for (let i = 0; i < testEvents.length; i++) {
        const event: NamespacedEvent = {
          ...
          timestamp: new Date(Date.now() - (5 - i) * 1000).toISOString(), // Spread over 5 seconds
    ...
    it('should filter events by timestamp range', async () => {
      const now = new Date();
      const fromTime = new Date(now.getTime() - 3000).toISOString(); // 3 seconds ago
      const toTime = new Date(now.getTime() - 1000).toISOString(); // 1 second ago

      const events = await eventStore.getEvents(undefined, fromTime, toTime);
    ...
    it('should combine pattern and timestamp filters', async () => {
      const now = new Date();
      const fromTime = new Date(now.getTime() - 4000).toISOString(); // 4 seconds ago

      const salesEvents = await eventStore.getEvents('sales.*', fromTime);
    ...
    it('should return empty array for out-of-range timestamps', async () => {
      ...
      const events = await eventStore.getEvents(undefined, futureTime);
      expect(events).toHaveLength(0);
    });
  });
