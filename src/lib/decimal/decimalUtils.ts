...
export class DecimalUtils {
   ...
   */
  static compare(a: DecimalInput, b: DecimalInput, domain?: 'tax' | 'inventory' | 'financial' | 'recipe'): number {
    const aDec = this.fromValue(a, domain);
    const bDec = this.fromValue(b, domain);
   ...
   * Verifica si a > b
   */
  static greaterThan(a: DecimalInput, b: DecimalInput, domain?: 'tax' | 'inventory' | 'financial' | 'recipe'): boolean {
    const aDec = this.fromValue(a, domain);
    const bDec = this.fromValue(b, domain);
   ...
   * Verifica si a >= b
   */
  static greaterThanOrEqualTo(a: DecimalInput, b: DecimalInput, domain?: 'tax' | 'inventory' | 'financial' | 'recipe'): boolean {
    const aDec = this.fromValue(a, domain);
    const bDec = this.fromValue(b, domain);
    return aDec.greaterThanOrEqualTo(bDec);
  }

  /**
   * Verifica si a < b
   */
  static lessThan(a: DecimalInput, b: DecimalInput, domain?: 'tax' | 'inventory' | 'financial' | 'recipe'): boolean {
    const aDec = this.fromValue(a, domain);
    const bDec = this.fromValue(b, domain);
    return aDec.lessThan(bDec);
  }

  /**
   * Verifica si a <= b
   */
  static lessThanOrEqualTo(a: DecimalInput, b: DecimalInput, domain?: 'tax' | 'inventory' | 'financial' | 'recipe'): boolean {
    const aDec = this.fromValue(a, domain);
    const bDec = this.fromValue(b, domain);
    return aDec.lessThanOrEqualTo(bDec);
  }

  /**
   * Verifica si a === b
   */
  static equals(a: DecimalInput, b: DecimalInput, domain?: 'tax' | 'inventory' | 'financial' | 'recipe'): boolean {
    const aDec = this.fromValue(a, domain);
    const bDec = this.fromValue(b, domain);
  ...
  /**
   * ⭐ CRITICAL - Convertir Markup a Margin
   * 
   * Fórmula: margin = markup / (1 + markup)
   * 
   * Ejemplo: 50% markup = 33.33% margin
   * 
   * ⚠️ IMPORTANTE: Markup y Margin NO son lo mismo
   ...
   */
  static convertMarkupToMargin(markupPercentage: DecimalInput): DecimalType {
    const markupDec = this.fromValue(markupPercentage, 'financial');
    
    // markup como decimal (50% = 0.5)
    const markupDecimal = markupDec.dividedBy(DECIMAL_CONSTANTS.HUNDRED);
    
    // margin = markup / (1 + markup)
    const marginDecimal = markupDecimal.dividedBy(DECIMAL_CONSTANTS.ONE.plus(markupDecimal));
    
    // Convertir de vuelta a porcentaje
    return marginDecimal.times(DECIMAL_CONSTANTS.HUNDRED);
  }

  /**
   * ⭐ CRITICAL - Convertir Margin a Markup
   * 
   * Fórmula: markup = margin / (1 - margin)
   * 
   * Ejemplo: 33.33% margin = 50% markup
   * 
   * ⚠️ IMPORTANTE: Margin NO puede ser 100% (precio = ganancia, costo = 0)
   * Si margin >= 100%, la fórmula produce infinito o error
   * 
   * Referencia: docs/teoria-administrativa/01-COSTOS-FUNDAMENTALES.md
   */
  static convertMarginToMarkup(marginPercentage: DecimalInput): DecimalType {
    const marginDec = this.fromValue(marginPercentage, 'financial');
    
    // Validar que margin < 100%
    if (marginDec.greaterThanOrEqualTo(DECIMAL_CONSTANTS.HUNDRED)) {
      throw new Error('Margin no puede ser >= 100% (markup sería infinito)');
    }
    
    // margin como decimal (33.33% = 0.3333)
    const marginDecimal = marginDec.dividedBy(DECIMAL_CONSTANTS.HUNDRED);
    
    // markup = margin / (1 - margin)
    const denominator = DECIMAL_CONSTANTS.ONE.minus(marginDecimal);
    
    if (denominator.isZero()) {
      throw new Error('Division by zero: margin = 100%');
    }
    
    const markupDecimal = marginDecimal.dividedBy(denominator);
    
    // Convertir de vuelta a porcentaje
    return markupDecimal.times(DECIMAL_CONSTANTS.HUNDRED);
  }

  /**
   * Calcular precio desde costo y margin deseado
   * 
   * Fórmula: precio = costo / (1 - margin)
   * 
   * Ejemplo: Costo $10, Margin 30% → Precio = $14.29
   */
  static calculatePriceFromMargin(cost: DecimalInput, marginPercentage: DecimalInput): DecimalType {
    const costDec = this.fromValue(cost, 'financial');
    const marginDec = this.fromValue(marginPercentage, 'financial');
    
    // Validar que margin < 100%
    if (marginDec.greaterThanOrEqualTo(DECIMAL_CONSTANTS.HUNDRED)) {
      throw new Error('Margin no puede ser >= 100%');
    }
    
    // margin como decimal
    const marginDecimal = marginDec.dividedBy(DECIMAL_CONSTANTS.HUNDRED);
    
    // precio = costo / (1 - margin)
    const denominator = DECIMAL_CONSTANTS.ONE.minus(marginDecimal);
    
    if (denominator.isZero() || denominator.isNegative()) {
      throw new Error('Invalid margin: results in zero or negative denominator');
    }
    
    return costDec.dividedBy(denominator);
  }
