...
export function calculateMenuEngineeringMatrix(
  ...
  // Calculate totals for index calculations
  const totalUnits = filteredData.reduce((sum, p) => sum + p.unitsSold, 0);
  const totalRevenue = filteredData.reduce((sum, p) => sum + p.totalRevenue, 0);
  const averageMargin = filteredData.reduce((sum, p) => sum + ((p.totalRevenue - p.totalCost) / p.totalRevenue), 0) / filteredData.length;
...
function calculateProductMenuData(
  ...
  const popularityIndex = (product.unitsSold / totalUnits) * 100;
  const profitMargin = (product.totalRevenue - product.totalCost) / product.totalRevenue;
  const profitabilityIndex = (profitMargin / averageMargin) * 100;
  const contributionMargin = product.totalRevenue - product.totalCost;
  const salesVelocity = product.unitsSold / config.analysisPeriodDays;
