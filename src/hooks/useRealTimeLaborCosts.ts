...
export function useRealTimeLaborCosts(options: UseRealTimeLaborCostsOptions = {}): UseRealTimeLaborCostsReturn {
  ...
  // Computed values
  const totalActiveCost = liveData.reduce((sum, employee) => {
    return employee.clock_in_time ? sum + employee.current_cost : sum;
  }, 0);

  const totalProjectedCost = liveData.reduce((sum, employee) => {
    return employee.clock_in_time ? sum + employee.projected_cost : sum;
  }, 0);

  const activeEmployeeCount = liveData.filter(employee => employee.clock_in_time !== null).length;

  const overtimeEmployeeCount = liveData.filter(employee => 
    employee.overtime_status === 'in_overtime' || employee.overtime_status === 'approaching'
  ).length;
...
export function useOvertimeMonitoring() {
  ...
  const overtimeEmployees = liveData.filter(employee => 
    employee.overtime_status !== 'none'
  );
...
export function useLiveCostDashboard(date?: string) {
  ...
  const departmentBreakdown = fullHook.liveData.reduce((acc, employee) => {
    if (!employee.clock_in_time) return acc;
    ...
    acc[dept].current_cost += employee.current_cost;
    acc[dept].projected_cost += employee.projected_cost;
    
    if (employee.overtime_status !== 'none') {
