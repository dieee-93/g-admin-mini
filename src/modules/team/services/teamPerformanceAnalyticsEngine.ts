...
export type TeamMemberStatus = 'active' | 'inactive' | 'on_leave' | 'terminated';
...
export interface DepartmentAnalytics {
  ...
  // Teaming Analysis
  underteamedShifts: number;       // Number of underteamed shifts
  overteamedShifts: number;        // Number of overteamed shifts
  optimalTeamingRate: number;      // % of optimally teamed shifts
...
export interface OrganizationMetrics {
  ...
  totalLaborCost: number;
  averageTenure: number;           // Average teamMember tenure in months
  turnoverRate: number;            // % turnover in period

  // Efficiency Metrics
  revenuePerTeamMember?: number;
  profitPerTeamMember?: number;
...
export interface StrategicInsight {
  ...
  category: 'performance' | 'cost' | 'teaming' | 'training' | 'retention';
  title: string;
  description: string;
  impact: 'high' | 'medium' | 'low';
  recommendation: string;
...
export interface TeamAnalyticsResult {
  ...
  periodStart: string;
  periodEnd: string;
  totalTeamMembersAnalyzed: number;
  
  // Individual Performance
  teamMemberMetrics: PerformanceMetrics[];
  ...
  recommendations: Array<{
    id: string;
    type: 'performance_improvement' | 'cost_optimization' | 'teaming_adjustment' | 'training_program' | 'retention_strategy';
...
export interface TeamAnalyticsConfig {
  ...
  includeRevenuePerTeamMember: boolean;
  ...
  industryBenchmarks?: {
    ...
    averageRevenuePerTeamMember: number;
  };
}
...
export class TeamPerformanceAnalyticsEngine {
  
  // Default configuration optimized for restaurant operations
  private static readonly DEFAULT_CONFIG: TeamAnalyticsConfig = {
    ...
    includeRevenuePerTeamMember: false,   // Requires sales integration
    ...
    industryBenchmarks: {
      ...
      averageRevenuePerTeamMember: 65000 // Annual revenue per teamMember
    }
  };
  ...
  /**
   * Generates comprehensive team performance analytics
   */
  static async generateTeamAnalytics(
    teamMembers: TeamMember[],
    timeEntries: TimeEntry[],
    schedules: Schedule[],
    config: Partial<TeamAnalyticsConfig> = {}
    ...
    // 1. Calculate individual teamMember metrics
    const teamMemberMetrics = await Promise.all(
      teamMembers
        .filter(emp => emp.status === 'active')
        .map(teamMember => this.calculateTeamMemberMetrics(
          teamMember, 
          filteredTimeEntries, 
          filteredSchedules, 
          fullConfig,
    ...
    // Filter teamMembers with sufficient data
    const validTeamMemberMetrics = teamMemberMetrics.filter(
      metrics => metrics !== null
    ) as PerformanceMetrics[];
    
    // 2. Calculate department analytics
    const departmentAnalytics = this.calculateDepartmentAnalytics(
      validTeamMemberMetrics,
      teamMembers
    );
    ...
    return {
      ...
      periodStart,
      periodEnd,
      totalTeamMembersAnalyzed: validTeamMemberMetrics.length,
      teamMemberMetrics: validTeamMemberMetrics,
      departmentAnalytics,
      organizationMetrics,
      strategicInsights,
      recommendations,
  ...
  private static async calculateTeamMemberMetrics(
    teamMember: TeamMember,
    timeEntries: TimeEntry[],
    schedules: Schedule[],
    config: TeamAnalyticsConfig,
    periodStart: string,
    periodEnd: string
  ): Promise<PerformanceMetrics | null> {
    
    const teamMemberTimeEntries = timeEntries.filter(entry => entry.teamMember_id === teamMember.id);
    const teamMemberSchedules = schedules.filter(schedule => schedule.teamMember_id === teamMember.id);
    
    // Skip if insufficient data
    if (teamMemberSchedules.length < config.minShiftsForAnalysis) {
      return null;
    }
    
    // Calculate time & attendance metrics
    const attendanceMetrics = this.calculateAttendanceMetrics(teamMemberTimeEntries, teamMemberSchedules);
    
    // Calculate productivity metrics
    const productivityMetrics = this.calculateProductivityMetrics(teamMember, teamMemberTimeEntries);
    
    // Calculate performance indicators
    const performanceIndicators = this.calculatePerformanceIndicators(teamMember, attendanceMetrics, productivityMetrics);
    
    // Calculate financial impact
    const financialMetrics = this.calculateFinancialMetrics(teamMember);

    // Determine performance trend
    const performanceTrend = this.determinePerformanceTrend(teamMember);
    ...
    return {
      ...
      skillsAcquired: [],
      retentionRisk: this.assessRetentionRisk(teamMember, attendanceMetrics, performanceIndicators),
  ...
  private static calculateAttendanceMetrics(timeEntries: TimeEntry[], schedules: Schedule[]) {
    ...
    const onTimeArrivals = timeEntries.filter(entry => {
      ...
      const scheduledShift = schedules.find(s => s.teamMember_id === entry.teamMember_id);
  ...
  private static calculateProductivityMetrics(teamMember: TeamMember, timeEntries: TimeEntry[]) {
    // For demonstration - in real system these would come from task tracking
    const baseProductivity = 70 + (teamMember.performance_rating - 3) * 10;
    const experienceBonus = this.calculateExperienceBonus(teamMember.hire_date);
    ...
    const taskCompletionRate = Math.min(100, 85 + (teamMember.performance_rating - 3) * 5);
    const averageTaskTime = 45 + Math.random() * 30; // 45-75 minutes average
    const qualityScore = Math.min(100, 75 + teamMember.performance_rating * 5);
  ...
  private static calculatePerformanceIndicators(teamMember: TeamMember, attendanceMetrics: AttendanceMetrics, productivityMetrics: ProductivityMetrics) {
    const performanceRating = teamMember.performance_rating;
  ...
  private static calculateFinancialMetrics(teamMember: TeamMember) {
    const laborCostPerHour = teamMember.hourly_rate * 1.3; // Include benefits/overhead
    ...
    const revenuePerHour = teamMember.department === 'Servicio' ? teamMember.hourly_rate * 4 : undefined;
  ...
  private static determinePerformanceTrend(teamMember: TeamMember): 'improving' | 'stable' | 'declining' {
    ...
    const recentPerformance = teamMember.performance_rating;
  ...
  private static assessRetentionRisk(teamMember: TeamMember, attendanceMetrics: AttendanceMetrics, performanceIndicators: Pick<PerformanceMetrics, 'performanceRating'>): 'low' | 'medium' | 'high' {
    let riskScore = 0;
    ...
    // Tenure factor
    const tenureMonths = this.calculateTenureMonths(teamMember.hire_date);
    if (tenureMonths < 6) riskScore += 1; // New teamMembers higher risk
  ...
  private static calculateDepartmentAnalytics(
    ...
    const departments = Array.from(new Set(allTeamMembers.map(emp => emp.department)));
    
    return departments.map(department => {
      const deptTeamMembers = allTeamMembers.filter(emp => emp.department === department);
      const deptMetrics = teamMemberMetrics.filter(metrics => metrics.department === department);
      
      if (deptMetrics.length === 0) {
        return this.createEmptyDepartmentAnalytics(department, deptTeamMembers.length);
      }
      ...
      const topPerformers = deptMetrics
        ...
        .slice(0, 3)
        .map(metrics => ({
          teamMemberId: metrics.teamMemberId,
          name: metrics.teamMemberName,
      ...
      return {
        department,
        totalTeamMembers: deptTeamMembers.length,
        activeTeamMembers: deptTeamMembers.filter(emp => emp.status === 'active').length,
        ...
        underteamedShifts: 0, // Would require scheduling analysis
        overteamedShifts: 0,
        optimalTeamingRate: 85, // Mock value
  ...
  private static createEmptyDepartmentAnalytics(department: Department, totalTeamMembers: number): DepartmentAnalytics {
    return {
      department,
      totalTeamMembers,
      activeTeamMembers: 0,
  ...
  private static identifyDepartmentImprovements(metrics: PerformanceMetrics[]) {
    ...
    if (avgAttendance < 85) {
      improvements.push({
        ...
        affectedTeamMembers: metrics.filter(m => m.attendanceRate < 85).length
      });
    }
    ...
    if (avgProductivity < 75) {
      improvements.push({
        ...
        affectedTeamMembers: metrics.filter(m => m.productivityScore < 75).length
      });
    }
  ...
  private static calculateOrganizationMetrics(
    ...
    if (teamMemberMetrics.length === 0) {
      return this.createEmptyOrganizationMetrics();
    }
    
    const activeTeamMembers = allTeamMembers.filter(emp => emp.status === 'active');
    ...
    const totalLaborCost = teamMemberMetrics.reduce((sum, m) => sum + (m.laborCostPerHour * m.totalHoursWorked), 0);
    
    // Calculate average tenure
    const avgTenure = this.calculateAverage(
      activeTeamMembers.map(emp => this.calculateTenureMonths(emp.hire_date))
    );
    ...
    return {
      ...
      costPerServiceHour: totalLaborCost / Math.max(1, teamMemberMetrics.reduce((sum, m) => sum + m.totalHoursWorked, 0)),
      
      performanceTrend: this.determineOrganizationTrend(teamMemberMetrics, 'performance') as 'improving' | 'stable' | 'declining',
      costTrend: 'stable' as const, // Would require historical cost data
      productivityTrend: this.determineOrganizationTrend(teamMemberMetrics, 'productivity') as 'improving' | 'stable' | 'declining'
    };
  }
  ...
  private static generateStrategicInsights(
    ...
    if (highCostDepts.length > 0) {
      insights.push({
        ...
        recommendation: 'Review teaming levels and optimize scheduling efficiency',
        priority: 3
      });
    }
  ...
  private static generateRecommendations(
    ...
    const lowPerformers = teamMemberMetrics.filter(m => m.performanceRating < 3.0);
    if (lowPerformers.length > 0) {
      recommendations.push({
        ...
        description: 'Implement targeted performance improvement plans for underperforming teamMembers',
        affectedTeamMembers: lowPerformers.map(m => m.teamMemberId),
        affectedDepartments: Array.from(new Set(lowPerformers.map(m => m.department))),
        estimatedImpact: 25,
        timeframe: 'medium_term' as const,
        actionSteps: [
          'Conduct performance reviews with affected teamMembers',
    ...
    const skillGaps = this.identifySkillGaps(teamMemberMetrics);
    if (skillGaps.length > 0) {
      recommendations.push({
        ...
        affectedTeamMembers: teamMemberMetrics.map(m => m.teamMemberId),
        affectedDepartments: Array.from(new Set(teamMemberMetrics.map(m => m.department))),
        estimatedImpact: 15,
        estimatedCost: 5000,
        timeframe: 'medium_term' as const,
        actionSteps: [
  ...
  private static generatePerformanceAlerts(
    ...
    teamMemberMetrics.forEach(metrics => {
      // Low performance alert
      if (metrics.performanceRating < config.lowPerformanceThreshold) {
        alerts.push({
          teamMemberId: metrics.teamMemberId,
          teamMemberName: metrics.teamMemberName,
      ...
      if (metrics.overtimeHours > config.overtimeThreshold) {
        alerts.push({
          ...
          recommendedActions: [
            ...
            'Consider additional teaming',
            'Monitor teamMember wellbeing'
          ]
        });
      }
      ...
      if (metrics.retentionRisk === 'high') {
        alerts.push({
          ...
          message: 'TeamMember shows high risk of turnover',
          recommendedActions: [
  ...
  private static identifySkillGaps(metrics: PerformanceMetrics[]): string[] {
    ...
    const lowProductivityTeamMembers = metrics.filter(m => m.productivityScore < 70);
    if (lowProductivityTeamMembers.length > 0) {
      gaps.push('Productivity Enhancement');
    }
    
    const lowQualityTeamMembers = metrics.filter(m => m.qualityScore < 75);
    if (lowQualityTeamMembers.length > 0) {
