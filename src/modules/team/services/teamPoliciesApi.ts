 ...
 * 
 * Supabase API layer for team_policies table
...
export interface UpdateTeamPoliciesInput {
  departments?: Department[];
  positions?: Position[];
  default_hourly_rates?: Record<string, number>;
  overtime_enabled?: boolean;
 ...
 */
export async function fetchTeamPolicies(): Promise<TeamPolicies[]> {
  ...
  if (error) {
    logger.error('TeamPoliciesApi', 'Failed to fetch team policies', error);
    throw error;
  }
...
/**
 * Fetch team policies by ID
 */
export async function fetchTeamPoliciesById(id: string): Promise<TeamPolicies> {
  const { data, error } = await supabase
    .from('team_policies')
    .select('*')
    .eq('id', id)
  ...
  if (error) {
    logger.error('TeamPoliciesApi', 'Failed to fetch team policies by ID', { id, error });
    throw error;
  }
 ...
 */
export async function fetchSystemTeamPolicies(): Promise<TeamPolicies> {
  const { data, error } = await supabase
    .from('team_policies')
    .select('*')
    .eq('is_system', true)
  ...
  if (error) {
    logger.error('TeamPoliciesApi', 'Failed to fetch system team policies', error);
    throw error;
  }
...
export async function updateTeamPolicies(
  id: string,
  updates: UpdateTeamPoliciesInput
): Promise<TeamPolicies> {
  const { data, error } = await supabase
    .from('team_policies')
    .update(updates)
    .eq('id', id)
  ...
  if (error) {
    logger.error('TeamPoliciesApi', 'Failed to update team policies', { id, error });
    throw error;
  }

  logger.info('TeamPoliciesApi', 'Team policies updated successfully', { id });
  return data;
}
 ...
 */
export async function toggleOvertime(id: string, enabled: boolean): Promise<TeamPolicies> {
  const { data, error } = await supabase
    .from('team_policies')
  ...
  if (error) {
    logger.error('TeamPoliciesApi', 'Failed to toggle overtime', { id, enabled, error });
    throw error;
  }

  logger.info('TeamPoliciesApi', 'Overtime toggled successfully', { id, enabled });
  return data;
}
...
export async function toggleCertificationTracking(
  ...
  if (error) {
    logger.error('TeamPoliciesApi', 'Failed to toggle certification tracking', { id, enabled, error });
    throw error;
  }

  logger.info('TeamPoliciesApi', 'Certification tracking toggled successfully', { id, enabled });
  return data;
}
...
export async function toggleShiftSwapApproval(
  ...
  if (error) {
    logger.error('TeamPoliciesApi', 'Failed to toggle shift swap approval', { id, required, error });
    throw error;
  }

  logger.info('TeamPoliciesApi', 'Shift swap approval toggled successfully', { id, required });
  return data;
}
