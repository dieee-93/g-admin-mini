...
import { useTeamStore } from '../staffStore';
import type { TeamMember, Schedule, TimeEntry } from '../staffStore';
...
describe('Staff Store', () => {
  ...
  describe('Initial State', () => {
    ...
    it('should provide all required actions', () => {
      ...
      expect(typeof result.current.updateTeamMember).toBe('function');
      expect(typeof result.current.deleteTeamMember).toBe('function');
  ...
  describe('Staff Management', () => {
    ...
    it('should load staff successfully', async () => {
      mockStaffApi.getTeamMembers.mockResolvedValueOnce([mockEmployee]);
      ...
      expect(mockStaffApi.getTeamMembers).toHaveBeenCalledOnce();
    });
    ...
    it('should handle loading state during staff fetch', async () => {
      ...
      mockStaffApi.getTeamMembers.mockReturnValueOnce(promise);
    ...
    it('should handle errors during staff fetch', async () => {
      ...
      mockStaffApi.getTeamMembers.mockRejectedValueOnce(new Error(errorMessage));
    ...
    it('should add teamMember successfully', async () => {
      ...
      const createdEmployee = { id: '2', ...newTeamMemberData };
      mockStaffApi.createTeamMember.mockResolvedValueOnce(createdEmployee);

      const { result } = renderHook(() => useTeamStore());

      await act(async () => {
        await result.current.addEmployee(newTeamMemberData);
      });

      expect(result.current.staff).toContainEqual(createdEmployee);
      expect(mockStaffApi.createTeamMember).toHaveBeenCalledWith(newTeamMemberData);
    });

    it('should update teamMember successfully', async () => {
      const updatedEmployee = { ...mockEmployee, hourly_rate: 30.0 };
      mockStaffApi.updateTeamMember.mockResolvedValueOnce(updatedEmployee);

      const { result } = renderHook(() => useTeamStore());

      // Set initial state with teamMember
      act(() => {
        result.current.setState({ staff: [mockEmployee] });
      });

      await act(async () => {
        await result.current.updateTeamMember('1', { hourly_rate: 30.0 });
      });

      expect(result.current.staff[0]).toEqual(updatedEmployee);
      expect(mockStaffApi.updateTeamMember).toHaveBeenCalledWith('1', { hourly_rate: 30.0 });
    });

    it('should delete teamMember successfully', async () => {
      mockStaffApi.deleteTeamMember.mockResolvedValueOnce(null);
      ...
      expect(result.current.staff).toEqual([]);
      expect(mockStaffApi.deleteTeamMember).toHaveBeenCalledWith('1');
    });

    it('should handle optimistic updates correctly', async () => {
      const newTeamMemberData = { name: 'Test TeamMember' };
      ...
      mockStaffApi.createTeamMember.mockReturnValueOnce(promise);

      const { result } = renderHook(() => useTeamStore());

      act(() => {
        result.current.addEmployee(newTeamMemberData);
      });

      // Should show loading state
      expect(result.current.loading).toBe(true);

      const createdEmployee = { id: '123', ...newTeamMemberData };
      await act(async () => {
    ...
    it('should calculate on-duty count based on current time entries', () => {
      ...
      act(() => {
        result.current.setState({ 
          ...
          staff: [{ id: '1' }, { id: '2' }] as TeamMember[]
        });
      });

      const stats = result.current.getStaffStats();

      expect(stats.onDuty).toBe(1); // Only teamMember 1 is still clocked in
    });
  });
  ...
  describe('Error Handling', () => {
    it('should handle network errors gracefully', async () => {
      ...
      mockStaffApi.getTeamMembers.mockRejectedValueOnce(networkError);
    ...
    it('should clear previous errors on successful operations', async () => {
      ...
      mockStaffApi.getTeamMembers.mockResolvedValueOnce([]);
