 ...
 * - Integrates with useMaterials hook for server state
...
export function useSmartInventoryAlerts() {
  ...
  // 🔧 FIX: Get materials from useMaterials hook (store only has UI state after refactor)
  ...
  useEffect(() => {
    ...
    if (materials && materials.length > 0 && timeSinceLastGeneration >= MIN_GENERATION_INTERVAL) {
