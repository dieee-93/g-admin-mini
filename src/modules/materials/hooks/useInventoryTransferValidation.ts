...
interface UseInventoryTransferValidationResult {
  ...
  validateField: (field: keyof InventoryTransferFormData, value: unknown) => void;
...
export function useInventoryTransferValidation(
  ...
  _existingTransfers: unknown[] = [],
  _currentTransferId?: string,
  ...
  // ===== VALIDATE FIELD =====
  const validateField = useCallback((field: keyof InventoryTransferFormData, value: unknown) => {
    form.setValue(field, value as InventoryTransferFormData[typeof field], { shouldValidate: true });
  }, [form]);
