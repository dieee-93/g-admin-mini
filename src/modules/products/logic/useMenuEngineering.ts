  ...
  const calculateMatrix = useCallback(async () => {
    try {
      ...
      if (salesData.length === 0) {
        ...
        const mockSalesData: ProductSalesData[] = [
        ...
        const calculatedMatrix = calculateMenuEngineeringMatrix(mockSalesData, config);
        setMatrix(calculatedMatrix);
        
        notify.info({
          title: "Usando datos de ejemplo",
          description: "Se están mostrando datos de ejemplo. Conecte con datos reales de ventas."
        });
      } else {
