...
export function useRealTimeLaborCosts(options: UseRealTimeLaborCostsOptions = {}): UseRealTimeLaborCostsReturn {
  ...
  const activeEmployeeCount = liveData.filter(teamMember => teamMember.clock_in_time !== null).length;

  const overtimeEmployeeCount = liveData.filter(teamMember =>
...
export function useOvertimeMonitoring() {
  ...
  const overtimeEmployees = liveData.filter(teamMember =>
