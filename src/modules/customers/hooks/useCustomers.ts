...
export function useCustomers() {
  ...
  const customers = useMemo<Customer[]>(() => {
    return crud.items
      // Filter out inactive customers (soft deleted)
  ...
  return {
    ...
    removeCustomer: async (id: string): Promise<void> => {
      ...
      console.log('🗑️ [useCustomers] Soft deleting customer:', id);
      ...
      try {
        ...
        if (error) {
          console.error('❌ [useCustomers] Soft delete failed:', error);
          throw error;
        }

        console.log('✅ [useCustomers] Customer soft deleted successfully');

        // Refresh the list to remove the inactive customer from view
        await crud.refresh();
      } catch (error) {
        console.error('❌ [useCustomers] Error in soft delete:', error);
        throw error;
      }
    },
...
export function useCustomerSearch() {
  ...
  const search = async (searchQuery: string) => {
    ...
    return searchCustomers(searchQuery, ['name', 'email', 'phone', 'mobile', 'dni']);
  };
